...
- **`docker compose up --build` falha no `npm install` compilando `optipng` (Mac com Apple Silicon / ARM):** os pacotes `optipng-bin`, `jpegtran-bin` e `jpegoptim-bin` (dependências do YellowLabTools) só têm binário pronto para x86/x64 e tentavam compilar do código-fonte em arm64. Desde a v1.2.1 eles são substituídos por stubs que usam os binários do sistema instalados via apt (ver [scripts/bin-stubs/README.md](scripts/bin-stubs/README.md)). Se ainda aparecer, confirme que está na versão mais recente (`git pull`) e reconstrua sem cache: `docker compose build --no-cache app && docker compose up -d`.
